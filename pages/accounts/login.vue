<template>
    <div class="max-w-md w-full p-6">
        ...
        <form @submit.prevent="login" class="space-y-4">
            ...
            <div>
                <button :disabled="!isValid || loading" :class="{ 'bg-gray-400 cursor-not-allowed': !isValid || loading }"
                    type="submit"
        ...
        <div class="mt-4 flex flex-col lg:flex-row items-center justify-between gap-4">
            <SocialSignInButton buttonType="button" :logoSrc="'/googleLogo.svg'" buttonText="Iniciar sesion con Google" />
            <SocialSignInButton buttonType="button" :logoSrc="'/githubLogo.svg'" buttonText="Iniciar sesion con Github" />
        </div>
        <div class="mt-4 text-sm text-gray-600 text-center">
            <router-link to="/accounts/reset" class="text-black hover:underline">
